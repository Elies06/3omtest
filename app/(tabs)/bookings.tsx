...
// // VERSION CORRIGÉE FINALE : Utilise listing_id, jointure pool_listings
...
// // Interface pour les détails du listing récupérés via la jointure
// interface SwimmerBookingPoolListing {
//     id: string;
//     title: string;
//     location: string | null;
//     pool_images: SwimmerBookingPoolImage[] | null; // Note: pool_images est le nom de la table liée
// }
...
//     created_at: string;
//     guest_count: number;
//     listing_id: string; // CORRIGÉ: de pool_id à listing_id
//     listing?: SwimmerBookingPoolListing | null; // CORRIGÉ: de pool à listing, utilise la nouvelle interface
...
//      useEffect(() => { if (fontsLoaded && !loadingBookings && !isLoadingAuth) { headerOpacity.value = withTiming(1, { duration: 500 }); bookingsOpacity.value = withTiming(1, { duration: 700 }); } }, [fontsLoaded, loadingBookings, isLoadingAuth]); // isLoadingAuth ajouté
...
//                  // *** VÉRIFIEZ CE CHEMIN PAR RAPPORT À VOTRE STRUCTURE DE FICHIERS app/ ***
...
//                  // Ne recharge que si nécessaire (premier chargement ou si liste vide)
//                  if(bookings.length === 0 || !isInitialLoadTriggered.current) {
//                      loadBookings();
//                      isInitialLoadTriggered.current = true; // Marquer le premier chargement
//                  }
//              } else {
...
//              }
//         }, [user, sessionInitialized, isLoadingAuth, isHost, userRoles, router, bookings.length]) // bookings.length ajouté
//     );
//     const isInitialLoadTriggered = useRef(false); // Pour gérer le premier chargement de useFocusEffect

//     // --- Charger les réservations de l'utilisateur (NAGEUR) ---
//     const loadBookings = useCallback(async (isRefresh = false) => {
//         if (!user || isHost) { // Ne pas charger si hôte ou non connecté
//             setBookings([]); setLoadingBookings(false); if (isRefresh) setRefreshing(false);
//             console.log("[loadBookings] Sautillé: Utilisateur hôte ou non connecté.");
//             return;
//         }
...
//                 .from('bookings')
//                 .select(`
//                     id, start_time, end_time, status, total_price, created_at, guest_count,
//                     listing_id,  -- Sélectionner listing_id
//                     listing:pool_listings ( -- Joindre avec pool_listings (nom interne)
//                         id,
//                         title,
//                         location,
//                         pool_images (url, position) -- Sélectionner images liées au listing
//                     ),
//                     conversation:conversations ( id, status ) -- Gardé tel quel
//                 `)
//                 .eq('user_id', user.id) // Filtrer par l'utilisateur actuel (nageur)
...
//                  ...booking,
//                  listing: booking.listing ? { // Utilise la propriété 'listing' définie dans le select
//                       ...booking.listing,
//                       // Trier les images ici si la relation ne le fait pas déjà
//                       pool_images: booking.listing.pool_images?.sort((a: any, b: any) => (a.position ?? 99) - (b.position ?? 99)) || null
//                   } : null,
//                  // Gérer la conversation si c'est un tableau (relation one-to-many)
//                  conversation: Array.isArray(booking.conversation) ? booking.conversation[0] : booking.conversation
//              }));

//             setBookings(formattedBookings as SwimmerBooking[]); // Appliquer le type corrigé
//         } catch (err: any) {
...
//         }
//     }, [user, isHost]); // isHost ajouté aux dépendances

//     // --- Effets ---
//      useEffect(() => {
//          // S'assurer que si l'utilisateur se déconnecte, la liste est vidée
//          if (sessionInitialized && !user) {
//              setLoadingBookings(false);
//              setBookings([]);
//              isInitialLoadTriggered.current = false; // Réinitialiser pour prochain login
//          }
//      }, [sessionInitialized, user]);

//     // Refresh (inchangé)
//     const onRefresh = useCallback(() => { setRefreshing(true); if (user && !isHost) { loadBookings(true); } else { setRefreshing(false); } }, [loadBookings, user, isHost]);

//     // Filtrage (inchangé)
//     const filteredBookings = useMemo(() => { if (!activeFilter) return bookings; return bookings.filter(booking => booking.status === activeFilter); }, [bookings, activeFilter]);

//     // Stats (inchangé)
//     const bookingsStats = useMemo(() => ({ total: bookings.length, upcoming: bookings.filter(b => { try { return isAfter(parseISO(b.start_time), new Date()) && ['pending', 'confirmed'].includes(b.status); } catch { return false; } }).length, completed: bookings.filter(b => b.status === 'completed').length }), [bookings]);

//     // Annulation (inchangé)
//     const handleOpenCancelModal = useCallback((bookingId: string) => { setBookingToCancelId(bookingId); setIsCancelModalVisible(true); }, []);
//     const confirmCancellation = useCallback(async () => { if (!user || !bookingToCancelId) return; setIsCancelModalVisible(false); setActionLoading(bookingToCancelId); try { const { error: updateError } = await supabase.from('bookings').update({ status: 'canceled' }).eq('id', bookingToCancelId).eq('user_id', user.id); if (updateError) throw updateError; await loadBookings(true); /* Passer true pour forcer refresh silencieux */ Alert.alert("Annulation confirmée", "Votre réservation a été annulée."); } catch (err: any) { console.error("Error cancelling booking:", err); Alert.alert("Erreur", `Impossible d'annuler : ${err.message || 'Erreur inconnue'}`); } finally { setActionLoading(null); setBookingToCancelId(null); } }, [user, bookingToCancelId, loadBookings]);


//     // --- Rendu d'un item de réservation (CORRIGÉ pour utiliser item.listing) ---
//     const renderBookingItem = useCallback(({ item, index }: { item: SwimmerBooking, index: number }) => {
//         try {
//             // Validation des dates (inchangée)
//             if (!item.start_time || !item.end_time) { return <View style={styles.bookingCard}><Text style={styles.errorTextSmall}>Données invalides</Text></View>; }
//             let startDate, endDate;
//             try { startDate = parseISO(item.start_time); endDate = parseISO(item.end_time); }
//             catch (err) { console.error("Erreur parse date pour item", item.id, err); return <View style={styles.bookingCard}><Text style={styles.errorTextSmall}>Date invalide pour résa {item.id}</Text></View>; }
//             if (!isValid(startDate) || !isValid(endDate)) { return <View style={styles.bookingCard}><Text style={styles.errorTextSmall}>Date invalide pour résa {item.id}</Text></View>; }
...
//             // Styles et labels status (inchangé)
...
//             // CORRIGÉ: Accès aux données du listing
...
//                     {/* Header avec image et titre (CORRIGÉ) */}
//                     <View style={styles.cardHeader}>
//                         {coverImage ? ( <Image source={{ uri: coverImage }} style={styles.propertyImage} resizeMode="cover" /> ) : ( <View style={[styles.propertyImage, styles.placeholderImage]}><CalendarIcon size={28} color="#9ca3af" /></View> )}
...
//                                     {listingLocation && (<View style={styles.locationContainer}><MapPin size={12} color="#ffffff" style={{opacity: 0.8}}/><Text style={styles.propertyLocation} numberOfLines={1}>{listingLocation}</Text></View>)}
//                                 </View>
//                                 <LinearGradient colors={statusBg} start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }} style={styles.statusBadge}>
//                                     {/* Icônes statut inchangées */}
...
//                         </View>
//                     </View>
//                      {/* Tags spéciaux (inchangés) */}
//                     {(isUpcoming || isTodayBooking) && ( <View style={[styles.specialTag, isTodayBooking ? styles.todayTag : styles.upcomingTag]}><Text style={[styles.specialTagText, isTodayBooking ? styles.todayTagText : styles.upcomingTagText]}>{isTodayBooking ? "Aujourd'hui" : "À venir"}</Text></View> )}
//                     {/* Infos réservation (inchangées) */}
//                     <View style={styles.bookingInfo}>
//                         <View style={styles.infoSection}>
//                             <View style={styles.detailRow}><CalendarIcon size={16} color="#475569" /><Text style={styles.detailLabel}>Date:</Text><Text style={styles.detailValue}>{formattedDay}</Text></View>
//                             <View style={styles.detailRow}><Clock size={16} color="#475569" /><Text style={styles.detailLabel}>Horaire:</Text><Text style={styles.detailValue}>{formattedStart} - {formattedEnd} ({durationHours > 0 ? `${durationHours}h` : '<1h'})</Text></View>
//                             <View style={styles.detailRow}><Users size={16} color="#475569" /><Text style={styles.detailLabel}>Invités:</Text><Text style={styles.detailValue}>{item.guest_count} personne{item.guest_count > 1 ? 's' : ''}</Text></View>
//                         </View>
...
//                         </View>
//                     </View>
//                     {/* Actions réservation (inchangées) */}
//                     <View style={styles.bookingActions}>
//                          {canCancel && ( <TouchableOpacity style={[styles.actionButton, styles.cancelButton, isItemActionLoading && styles.disabledButton]} onPress={() => handleOpenCancelModal(item.id)} disabled={isItemActionLoading}> {actionLoading === item.id ? <ActivityIndicator size="small" color="#b91c1c"/> : <XCircle size={16} color="#b91c1c" />} <Text style={styles.cancelButtonText}>Annuler</Text> </TouchableOpacity> )}
//                          <TouchableOpacity style={[styles.actionButton, styles.messageButton, (!canContact || isItemActionLoading) && styles.disabledButton]} onPress={() => { if (canContact && item.conversation) { router.push(`/(tabs)/conversations/${item.conversation.id}`); } else { Alert.alert("Contacter", "Conversation non disponible."); } }} disabled={!canContact || isItemActionLoading}><MessageCircle size={16} color={canContact ? '#0891b2' : '#9ca3af'} /> <Text style={[styles.messageButtonText, !canContact && { color: '#9ca3af' }]}>Contacter</Text></TouchableOpacity>
//                          <TouchableOpacity style={[styles.actionButton, styles.detailsButton, isItemActionLoading && styles.disabledButton]} onPress={() => { if(item.id && typeof item.id === 'string') { router.push(`/my-bookings/${item.id}`); } else { Alert.alert("Erreur", "ID manquant."); } }} disabled={isItemActionLoading}><Info size={16} color="#475569" /> <Text style={styles.detailsButtonText}>Détails</Text></TouchableOpacity>
//                     </View>
//                 </Animated.View>
//             );
//         } catch (err) {
//             console.error("Erreur rendu item réservation:", item.id, err);
//             return ( <View style={styles.bookingCard}><Text style={styles.errorTextSmall}>Erreur affichage résa {item.id}</Text></View> );
//         }
//     }, [actionLoading, handleOpenCancelModal, router]);

//     // Header Component (inchangé)
//     const renderHeader = useCallback(() => { /* ... (code identique) ... */ if (isHost) return null; return ( <Animated.View style={[styles.header, headerAnimatedStyle]}><View style={styles.welcomeContainer}><Text style={styles.welcomeTitle}>Mes réservations</Text><Text style={styles.welcomeSubtitle}>Suivez et gérez vos prochaines baignades.</Text></View><View style={styles.statsContainer}><View style={styles.statItem}><Text style={styles.statNumber}>{bookingsStats.total}</Text><Text style={styles.statLabel}>Total</Text></View><View style={styles.statDivider} /><View style={styles.statItem}><Text style={styles.statNumber}>{bookingsStats.upcoming}</Text><Text style={styles.statLabel}>À venir</Text></View><View style={styles.statDivider} /><View style={styles.statItem}><Text style={styles.statNumber}>{bookingsStats.completed}</Text><Text style={styles.statLabel}>Terminées</Text></View></View><View style={styles.filterScrollContainer}><ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.filterContent}>{FILTERS.map((filter) => { const isActive = activeFilter === filter.key; return ( <TouchableOpacity key={filter.key || 'all'} style={[styles.filterButton, isActive && styles.filterButtonActive]} onPress={() => setActiveFilter(filter.key)}><View style={[styles.filterIconContainer, isActive && styles.filterIconContainerActive]}>{filter.icon()}</View><Text style={[styles.filterText, isActive && styles.filterTextActive]}>{filter.label}</Text></TouchableOpacity> ); })}</ScrollView></View></Animated.View> ); }, [headerAnimatedStyle, bookingsStats, activeFilter, isHost]);

//     // EmptyComponent (inchangé)
//     const renderEmptyComponent = useCallback(() => { /* ... (code identique) ... */ if (loadingBookings || bookingsError || isHost) return null; return ( <View style={styles.emptyContainer}><View style={styles.emptyIconContainer}><CalendarIcon size={50} color="#ffffff" /></View><Text style={styles.emptyTitle}>Aucune réservation</Text><Text style={styles.emptyText}>{activeFilter ? `Vous n'avez pas de réservation "${BOOKING_STATUS_LABELS[activeFilter] || activeFilter}".` : "Vous n'avez pas encore réservé de piscine."}</Text><TouchableOpacity style={styles.findButton} onPress={() => router.push('/search')}><Text style={styles.findButtonText}>Trouver une piscine</Text><ArrowRight size={18} color="#ffffff" /></TouchableOpacity></View> ); }, [loadingBookings, bookingsError, activeFilter, router, isHost]);

//     // --- Rendu Principal ---
//     if (!fontsLoaded || !sessionInitialized || isLoadingAuth) { return <SafeAreaView style={styles.loadingContainer}><ActivityIndicator size="large" color="#0891b2" /></SafeAreaView>; }
...
//     if (!user && sessionInitialized) { /* Ajout sessionInitialized pour éviter flash */ return <SafeAreaView style={styles.errorContainer}><Text style={styles.errorText}>Veuillez vous connecter.</Text></SafeAreaView>; }
//     if (isHost && sessionInitialized) { /* Ajout sessionInitialized */ return <SafeAreaView style={styles.loadingContainer}><ActivityIndicator size="large" color="#0891b2" /><Text style={styles.loadingText}>Redirection vers l'espace hôte...</Text></SafeAreaView>; }
...
//                 ListFooterComponent={ bookingsError && !loadingBookings ? ( <View style={styles.globalErrorContainer}><AlertCircle size={18} color="#b91c1c" style={{ marginRight: 8 }}/><Text style={styles.errorTextSmall}>{bookingsError}</Text></View> ) : ( <View style={{ height: 40 }} /> ) }
//                 refreshControl={<RefreshControl refreshing={refreshing} onRefresh={onRefresh} colors={['#0891b2']} tintColor="#0891b2" progressBackgroundColor="#ffffff"/>}
...
//                  </View>
//              )}
//             {/* Modal d'annulation (inchangée) */}
//              <Modal animationType="fade" transparent={true} visible={isCancelModalVisible} onRequestClose={() => { if (!actionLoading) { setIsCancelModalVisible(false); setBookingToCancelId(null); } }}>
...
//                                <TouchableOpacity style={[styles.modalButton, styles.modalButtonCancel, actionLoading && styles.disabledButton]} onPress={() => { setIsCancelModalVisible(false); setBookingToCancelId(null); }} disabled={actionLoading}>
...
//     // Styles pour le Modal (ajoutés/adaptés)
